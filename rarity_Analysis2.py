...
def TaxaWork(Taxa):
    ...
    #dfJenk = dfJenk.loc[dfJenk['BINOMIAL'] == 'Plethodon glutinosus']  # match on SciName
    
    for row in dfJenk.itertuples(index=True, name='Pandas'):
        # Set variables
        BINOMIAL = getattr(row,"BINOMIAL")
        Jend = getattr(row,"Jend")
        ...
        if len(dfGI.loc[dfGI['iucnSciName'] == BINOMIAL, 'gapSppCode']) == 1:
            ...
            intJend = Jend
            print(' Matched on iucnSciName : ' + BINOMIAL)
        # match multiple items in series and set intJend
        elif len(dfGI.loc[dfGI['iucnSciName'] == BINOMIAL, 'gapSppCode']) > 1:
            
IF ANY MULTIPLE RECORDS ARE SET UP THEN THE WRITING NEEDS TO HAPPEN AT THE END - CAN ALL THE VARIABLE SETTING BE MOVED TO THE END?
            ...
            try: 
                ...
                if jsonS["count"] > 0:
                    ...
                    if len(dfGI.loc[dfGI['iucnSciName'] == str(iucnAcceptedName), 'gapSppCode']) == 1:
                        ...
                        intJend = Jend
                        print(' Matched on iucnSynonym (' + iucnAcceptedName + ') : ' + BINOMIAL)
                    # if > 1 item in series, select first one
                        ...
                        try:
                            ...
                            intJend = Jend
                            print(' Matched on gapSciName 1: ' + BINOMIAL)
                        except:
                ...
                # if no results came back
                    ...
                    try:
                        strUC = dfGI.loc[dfGI['gapSciName'] == BINOMIAL, 'gapSppCode'].item()
                        intJend = Jend
                        print(' Matched on gapSciName 2 : ' + BINOMIAL)
                    except:
                        print(' Failed to match on iucnSciName, iucnSynonym or gapSciName 2 : ' + BINOMIAL)
            except:
